...
_FAIL_OPEN_COUNTERABLE_CODES = {"missing_year"}
...
class RecognitionGuard:
    ...
    def __init__(self, settings: RecognitionSettings, runtime: RecognitionRuntime | None = None):
        ...
        self.strategy = parse_strategy(self.settings.mode, self.settings.custom_config)
        # keyword_config 是内部测试/运行对象入口，不是公开配置键；用户 YAML 使用 custom_config。
        self.keyword_groups = (
            load_keyword_groups(self.settings.keyword_config)
            if self.settings.keyword_config and not self.settings.custom_config
            else self.strategy.keyword_groups
        )
        ...
        self._secondary_cache: dict[str, dict] = {}
    ...
    def filter_candidate_dicts(self, target_dict, candidate_dicts, contexts, *,
        ...
        fallback_reason = ""
        if not retained and decisions:
            recovered = []
            for context, decision in zip(context_items, decisions):
                if decision.final_action == ACTION_SOFT_BLOCK and self.strategy.is_recoverable(decision.code):
                    decision.final_action = ACTION_OBSERVE
                    recovered.append(context)
            if recovered:
                retained = recovered
                fallback_applied = True
                fallback_reason = "soft_block_empty_result_protection"
        batch = self._make_batch(
            decisions,
            retained,
            len(candidates),
    ...
    def notification_payload(self, subscribe):
        ...
        if not self.last_batch:
            return None
        notify_mode = self.settings.notify_mode or "off"
        if self.settings.mode == "off" or notify_mode == "off":
            return None
        notify_actions = {ACTION_BLOCK, ACTION_SOFT_BLOCK}
        if notify_mode == "all":
            notify_actions.add(ACTION_OBSERVE)
        decisions = [
            decision for decision in self.last_batch.decisions
            if decision.final_action in notify_actions
        ]
        if not decisions:
            return None
        first = decisions[0]
        ...
        lines = [
            f"拦截 {counts.get(ACTION_BLOCK, 0)} 条，"
            f"软拦截 {counts.get(ACTION_SOFT_BLOCK, 0)} 条，"
            f"观察 {counts.get(ACTION_OBSERVE, 0)} 条"
        ]
    ...
    def _decide_enabled(self, target: RecognitionTarget, candidate: CandidateResource,
        ...
        if hard_range_decision:
            ...
            return self._decision(action, code, reason, candidate, risk=_HARD_BLOCK)
        ...
        if soft_range_decision:
            soft_evidence.append(soft_range_decision)

        if soft_type_decision:
            soft_evidence.append(soft_type_decision)

        if block_match:
            action = self.strategy.action_for("user_block")
            soft_evidence.append((action, "user_block", "命中 block 关键字"))
        ...
        if secondary_decision:
            ...
            soft_evidence.append((action, code, reason))

        if candidate.year is None:
            action = self._missing_year_action()
            soft_evidence.append((action, "missing_year", "候选缺少年份，按当前模式记录"))

        if soft_evidence:
            return self._combine_soft_evidence(soft_evidence, candidate, allow_match)
    ...
    def _secondary_decision(self, target: RecognitionTarget, candidate: CandidateResource, secondary_failed: bool = False):
        ...
        action = self.strategy.action_for("secondary_identity_conflict")
    ...
    def _missing_year_action(self) -> str:
        return self.strategy.action_for("missing_year")
    ...
    def _combine_soft_evidence(self, evidence_items: list[tuple[str, str, str]], candidate: CandidateResource,
                               allow_match: str | None) -> Decision:
        """合并低/策略风险证据；用户策略 block 不伪装成 hard veto。"""
        fail_open_items = [item for item in evidence_items if item[0] == ACTION_FAIL_OPEN]
        independent_risk_items = [
            item
            for item in evidence_items
            if item[0] in {ACTION_SOFT_BLOCK, ACTION_BLOCK} and not self._can_fail_open_counter(item)
        ]
        if fail_open_items and not independent_risk_items:
            primary = fail_open_items[0]
        else:
            ranked_actions = [ACTION_ALLOW, ACTION_OBSERVE, ACTION_SOFT_BLOCK, ACTION_BLOCK]
            primary = max(
                [item for item in evidence_items if item[0] != ACTION_FAIL_OPEN],
                key=lambda item: ranked_actions.index(item[0]),
            )
        action, code, reason = primary
        decision = self._risk_or_allow(action, code, reason, candidate, allow_match)
        existing_codes = {item.code for item in decision.evidence}
        for item_action, item_code, item_reason in evidence_items:
            if item_code in existing_codes:
                continue
            decision.evidence.append(Evidence(
                group="recognition",
                code=item_code,
                level=self._risk_for_action(item_action),
                message=item_reason,
            ))
        return decision

    def _can_fail_open_counter(self, item: tuple[str, str, str]) -> bool:
        """二次识别 fail-open 只抵消模式模板自带的空结果风险，不覆盖用户显式策略。"""
        action, code, _reason = item
        return (
            code in _FAIL_OPEN_COUNTERABLE_CODES
            and action == ACTION_BLOCK
            and code not in self.strategy.explicit_actions
        )
    ...
    def _make_batch(self, decisions: list[Decision], retained: list, input_count: int, *,
                    selection_original_count=None, stage_counts=None, fallback_applied=False,
                    fallback_reason: str = "") -> BatchDecision:
        ...
        batch.fallback_reason = fallback_reason
    ...
    def _audit_summary(self, batch: BatchDecision) -> str:
        parts = [
            ...
            f"notify_mode={self.settings.notify_mode}",
            f"cache_maxsize={self.settings.cache_maxsize}",
            f"strategy={self.strategy.summary}",
            ...
            f"fallback_applied={str(batch.fallback_applied).lower()}",
        ]
        if batch.fallback_reason:
            parts.append(f"fallback_reason={batch.fallback_reason}")
