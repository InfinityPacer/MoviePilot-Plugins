...
插件 Release workflow 在打包前为对应代次的前端执行 frozen lockfile 安装和生产构建，并校验构建生成的 `dist/assets/remoteEntry.js` 存在。`dist/` 是本地和 Release 打包阶段生成的产物，不纳入 Git。
...
- `guard_veto`：完成证据尚不足以直接放行，需要观察。通知原因由实际裁决分支提供，不在通知层重新推导策略。
