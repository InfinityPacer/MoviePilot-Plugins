...
export function normalizeLocale(source: LocaleSource): SupportedLocale {
  ...
  const locale =
    normalized === 'zh-cn' ? 'zh-CN' : normalized === 'zh-tw' ? 'zh-TW' : normalized === 'en-us' ? 'en-US' : 'zh-CN'
...
export function localizeGroups(localeSource: LocaleSource, source: readonly GroupMeta[] = groups): GroupMeta[] {
  ...
  return source.map(group => {
    ...
    const [title, summary] =
      locale === 'zh-CN' ? [group.title, group.summary] : locale === 'zh-TW' ? translation.tw : translation.en
