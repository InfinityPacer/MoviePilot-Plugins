...
class TestEventOrdering:
    ...
    def test_episodes_refresh_label_uses_media_when_subscribe_missing(self):
        ...
        assert EventProxy._format_episodes_refresh_label(data) == "镖人 (2023) S1(id=32, media=themoviedb:325228, scene=refresh)"
    ...
    def test_episodes_refresh_label_uses_mediainfo_contract(self):
        ...
        data = SubscribeEpisodesRefreshEventData(
            ...
            mediainfo=MediaInfo(
                title="镖人", year="2023",
                media_source="themoviedb", media_id="325228",
            ),
            scene="refresh",
        )
...
class TestDomainGating:
    ...
    @pytest.mark.parametrize("media_type", [MediaType.MOVIE, MediaType.TV])
    def test_supported_media_completion_check_reaches_guard(self, media_type):
        """电影和电视剧完成检查继续交给完成守卫。"""
        guard = MagicMock()
        event = SimpleNamespace(event_data=SimpleNamespace(subscribe=_sub(type=media_type)))

        EventProxy(guard=guard).on_completion_check(event)

        guard.handle.assert_called_once_with(event)

    def test_music_entry_points_skip_all_downstream_modules(self):
        """音乐订阅在各外层入口拒绝，不进入生命周期、识别、清理或下载状态链路。"""
        subscribe = _sub(type=MediaType.MUSIC)
        ...
        site_refresh = MagicMock()
        pending_refresh = MagicMock()
        mediainfo_from_dict = MagicMock(return_value=SimpleNamespace())
        detect_backfill = MagicMock(return_value=[1])
        proxy = EventProxy(
            ...
            download_monitor=monitor,
            subscription_cleanup=cleanup,
            orchestrator=orchestrator,
            volatility=volatility,
            site_refresh=site_refresh,
            pending_refresh=pending_refresh,
            mediainfo_from_dict=mediainfo_from_dict,
            detect_backfill_episodes_fn=detect_backfill,
        )

        proxy.on_completion_check(SimpleNamespace(
            event_data=SimpleNamespace(subscribe=subscribe)
        ))
        from app.schemas.event import SubscribeEpisodesRefreshEventData
        proxy.on_episodes_refresh(SimpleNamespace(event_data=SubscribeEpisodesRefreshEventData(
            subscribe_id=1,
            current_total_episode=12,
        )))
        proxy.on_subscribe_added(SimpleNamespace(event_data={
            "subscribe_id": 1,
            "mediainfo": {"type": "音乐"},
        }))
        proxy.on_subscribe_modified(SimpleNamespace(event_data={
            "subscribe_id": 1,
            "fields": ["state", "best_version"],
            "subscribe_info": {"state": "R", "best_version": 1},
            "old_subscribe_info": {"state": "S", "best_version": 0},
        }))
        proxy.on_subscribe_complete(SimpleNamespace(event_data={
            "subscribe_id": 1,
            "subscribe_info": subscribe.__dict__,
            "mediainfo": {"type": "音乐"},
        }))
        selection_data = SimpleNamespace(
            origin='Subscribe|{"id": 1}',
            contexts=[SimpleNamespace(torrent_info=SimpleNamespace())],
            updated=False,
            updated_contexts=None,
            source="",
        )
        proxy.on_resource_selection(SimpleNamespace(event_data=selection_data))
        proxy.on_resource_download(SimpleNamespace(event_data=SimpleNamespace(
            origin='Subscribe|{"id": 1}',
            context=SimpleNamespace(torrent_info=SimpleNamespace(enclosure="music")),
            episodes=[],
            cancel=False,
        )))
        proxy.on_download_added(SimpleNamespace(event_data={
        ...
        guard.handle.assert_not_called()
        lifecycle.assert_not_called()
        priority.assert_not_called()
        verifier.assert_not_called()
        task_manager.assert_not_called()
        recognition_guard.assert_not_called()
        deletes_store.assert_not_called()
        monitor.assert_not_called()
        cleanup.assert_not_called()
        orchestrator.assert_not_called()
        volatility.assert_not_called()
        site_refresh.assert_not_called()
        pending_refresh.assert_not_called()
        mediainfo_from_dict.assert_not_called()
        detect_backfill.assert_not_called()
        assert selection_data.updated is False
