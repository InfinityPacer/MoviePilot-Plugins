...
class TestBuildScope:
    ...
    def test_main_season_scope(self, make_subscribe, make_mediainfo):
        ...
        sub = make_subscribe(media_id="100", season=1, episode_group=None)
    ...
    def test_episode_group_scope(self, make_subscribe, make_mediainfo):
        ...
        sub = make_subscribe(media_id="100", season=1, episode_group="eg-abc")
    ...
    def test_non_tmdb_identity_does_not_call_tmdb(self, make_subscribe, make_mediainfo):
        """非 TMDB 订阅没有可靠转换结果时不得把来源原生 ID 传给 TMDB API。"""
        calls = []
        sub = make_subscribe(media_source="douban", media_id="100", season=1)

        scope = build_scope(sub, make_mediainfo(), lambda *args, **kwargs: calls.append((args, kwargs)))

        assert scope.source == "tmdb_unavailable"
        assert scope.tmdbid is None
