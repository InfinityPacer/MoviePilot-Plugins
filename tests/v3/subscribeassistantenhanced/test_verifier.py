...
def _sub(media_id="100", season=1, episode_group=None, total=12, best_version=0, best_version_full=0):
    return SimpleNamespace(
        id=1, media_source="themoviedb", media_id=str(media_id), season=season, episode_group=episode_group,
...
class TestSnapshot:
    ...
    def test_saves_snapshot(self):
        ...
        assert len(snaps) == 1
        assert snaps[0]["media_source"] == "themoviedb"
        assert snaps[0]["media_id"] == "100"
    ...
    def test_dedup_by_key(self):
        """同 (media_source, media_id, season, episode_group_id) 幂等去重。"""
        store = {}
        v = _verifier(store)
        scope = SeasonScope(tmdbid=100, season=1, source="main_season")
...
class TestVerifyAll:
    ...
    def test_no_change_keeps_snapshot(self):
        ...
        store = {"snapshots": {"list": [{
            "media_source": "themoviedb", "media_id": "100", "season": 1, "episode_group_id": None,
    ...
    def test_cleanup_expired_uses_configured_retention_without_tmdb(self):
        ...
        store = {"snapshots": {"list": [
            ...
            {
                "media_source": "themoviedb", "media_id": "101", "season": 1, "episode_group_id": None,
                "total_at_completion": 12,
        ...
        assert [snap["media_id"] for snap in store["snapshots"]["list"]] == ["101"]
        tmdb_fn.assert_not_called()

    def test_scope_aware_group_verification(self):
        """group scope 快照用 group 集数验证。"""
        store = {"snapshots": {"list": [{
            "media_source": "themoviedb", "media_id": "100", "season": 1, "episode_group_id": "eg-1",
    ...
    def test_rebuild_deletes_best_version(self):
        ...
        existing_bv = SimpleNamespace(
            id=99, media_source="themoviedb", media_id="100", season=1, episode_group=None,
    ...
    def test_rebuild_exception_keeps_snapshot_and_continues_next_snapshot(self):
        ...
        assert [snap["media_id"] for snap in store["snapshots"]["list"]] == ["100"]
    ...
    def test_mixed_full_and_normal_subscribes_rebuilds_after_removing_only_full(self):
        ...
        normal_subscribe = SimpleNamespace(
            id=100, media_source="themoviedb", media_id="100", season=1, episode_group=None,
    ...
    def test_rebuild_does_not_touch_different_episode_group(self):
        ...
        store = {"snapshots": {"list": [{
            "media_source": "themoviedb", "media_id": "100", "season": 1, "episode_group_id": "eg-new",
            "total_at_completion": 12, "completed_at": time.time(),
            "subscribe_config": {"name": "测试"},
        }]}}
        other_group = SimpleNamespace(
            id=99, media_source="themoviedb", media_id="100", season=1,
    ...
    def test_covered_active_normal_subscribe_consumes_snapshot(self):
        ...
        existing = SimpleNamespace(
            id=50, media_source="themoviedb", media_id="100", season=1, episode_group=None,
