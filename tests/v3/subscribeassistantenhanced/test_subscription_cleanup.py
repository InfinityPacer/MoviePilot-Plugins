...
def _sub(**kwargs):
    ...
    defaults = dict(
        id=1, name="测试剧", media_source="themoviedb", media_id="100", season=1,
        episode_group=None,
        state="R", type="电视剧",
...
class TestHistoryClear:
    ...
    def _transfer_event(self, episode=1, media_source="themoviedb", media_id="100", season=1):
        """构造带目标文件集数的整理拦截事件。"""
    ...
    def test_resource_download_clear_deletes_src_and_emits_event(self):
        ...
        orch._get_histories = lambda media_source, media_id, mtype, season=None: [h]
        ...
        assert task["media_source"] == "themoviedb"
        assert task["media_id"] == "100"
    ...
    def test_migrate_snapshot_identities_is_idempotent(self):
        """旧 TMDB 快照只迁移一次，已有 V3 身份和无法识别的记录保持原样。"""
        store = {"subscription_cleanup_histories": {
            "legacy": {"tmdbid": 100, "season": "S01"},
            "canonical": {"media_source": "douban", "media_id": "200", "season": "S01"},
            "unknown": {"season": "S01"},
        }}
        saves = []
        ...
        def update(key, updater):
            ...
            saves.append(dict(store[key]))
            return store[key]

        orch = SubscriptionCleanup(task_data_update=update)

        assert orch.migrate_snapshot_identities() == 1
        assert store["subscription_cleanup_histories"] == {
            "legacy": {"media_source": "themoviedb", "media_id": "100", "season": "S01"},
            "canonical": {"media_source": "douban", "media_id": "200", "season": "S01"},
            "unknown": {"season": "S01"},
        }
        assert orch.migrate_snapshot_identities() == 0
        assert len(saves) == 2

    def test_migrate_snapshot_identity_save_failure_keeps_legacy_data(self):
        """底层持久化失败时不得先删除旧快照。"""
        store = {"subscription_cleanup_histories": {"legacy": {"tmdbid": 100}}}

        def update(key, updater):
            original = store[key]
            updated = updater({task_key: dict(task) for task_key, task in original.items()})
            assert updated["legacy"]["media_id"] == "100"
            raise RuntimeError("save failed")

        orch = SubscriptionCleanup(task_data_update=update)

        with pytest.raises(RuntimeError, match="save failed"):
            orch.migrate_snapshot_identities()
        assert store == {"subscription_cleanup_histories": {"legacy": {"tmdbid": 100}}}
    ...
    def test_tv_clear_type_processes_tv_subscription(self):
        ...
        orch._get_histories.assert_called_once_with("themoviedb", "100", "电视剧", "S01")
    ...
    def test_normal_subscription_directory_intercept_without_meta_is_ignored(self):
        ...
        event = SimpleNamespace(event_data=SimpleNamespace(
            cancel=False,
            mediainfo=SimpleNamespace(
                media_source="themoviedb", media_id="100", type=MediaType.TV, season=1,
            ),
    ...
    def test_dest_delete_exception_keeps_snapshot_and_skips_notification(self):
        """目标文件删除抛错时按失败处理，恢复快照且不发送已处理通知。"""
        store = {"subscription_cleanup_histories": {"task-e1": {
            "media_source": "themoviedb",
            "media_id": "100",
            "type": "电视剧",
        ...
        def delete_media_file(_fileitem):
    ...
    def test_history_clear_restores_claim_when_dest_clear_raises(self):
        """目标文件清理过程出现未捕获异常时，已 claim 的快照必须恢复。"""
        store = {"subscription_cleanup_histories": {"task-e1": {
            "media_source": "themoviedb",
            "media_id": "100",
            "type": "电视剧",
        ...
        def clear_transfer_dest_histories(_task):
            raise RuntimeError("unexpected")

        orch.clear_transfer_dest_histories = clear_transfer_dest_histories
    ...
    def test_failed_episode_restore_merges_with_remaining_snapshot(self):
        """按集 claim 后若删除失败，应把失败集并回仍保留的其他集快照。"""
        store = {"subscription_cleanup_histories": {"task-e1-e2": {
            "media_source": "themoviedb",
            "media_id": "100",
            "type": "电视剧",
        ...
        task = store["subscription_cleanup_histories"]["task-e1-e2"]
        assert task["target_episodes"] == [1, 2]
        assert task["histories"] == [
            {"dest_fileitem": {"path": "/dest/e2.mkv"}, "episodes": "E02"},
            {"dest_fileitem": {"path": "/dest/e1.mkv"}, "episodes": "E01"},
        ]
    ...
    def test_clear_history_task_helpers_keep_guard_semantics(self):
        """快照占用和恢复 helper 的空输入分支保持无副作用语义。"""
        orch = SubscriptionCleanup()
        orch._restore_clear_history_task({"_task_key": "task-e1"})
        ...
        orch._restore_clear_history_task({"histories": []})
        task = {"scene": "normal", "histories": [{"episodes": "E01"}]}
        assert orch._consume_clear_history_task(task, [])[0] is task
        assert orch._consume_clear_history_task(task, [])[1] is None
        assert orch._consume_clear_history_task({"scene": "normal", "histories": [{"episodes": "E02"}]}, [1])[0]["histories"] == [
            {"episodes": "E02"}
        ]

        assert store == {}

    def test_history_identity_uses_id_or_serialized_paths(self):
        """失败恢复去重优先使用整理记录 id，缺少 id 时使用源/目标路径和集数组合。"""
        assert SubscriptionCleanup._history_identity({"id": "1", "episodes": "E01"}) == "id:1"
        assert SubscriptionCleanup._history_identity({
            "src_fileitem": {"path": "/src/e1.mkv"},
            "dest_fileitem": {"path": "/dest/e1.mkv"},
            "episodes": "E01",
        }) == "/src/e1.mkv|/dest/e1.mkv|E01"
        assert SubscriptionCleanup._history_identity(SimpleNamespace(id="2")) == "id:2"
        assert SubscriptionCleanup._history_identity(SimpleNamespace(
            src_fileitem={"path": "/src/e2.mkv"},
            dest_fileitem={"path": "/dest/e2.mkv"},
            episodes="E02",
        )) == "/src/e2.mkv|/dest/e2.mkv|E02"
    ...
    def test_transfer_intercept_tv_task_requires_matching_season(self):
        ...
        event = SimpleNamespace(event_data=SimpleNamespace(
            cancel=False,
            mediainfo=SimpleNamespace(
                media_source="themoviedb", media_id="100", type=MediaType.TV,
            ),
    ...
    def test_transfer_intercept_without_snapshot_returns_false(self):
        ...
        event = SimpleNamespace(event_data=SimpleNamespace(
            cancel=False,
            mediainfo=SimpleNamespace(media_source="themoviedb", media_id="100"),
        ))
    ...
    def test_transfer_intercept_clear_returns_true(self):
        ...
        store = {"subscription_cleanup_histories": {"task-1": {
            "media_source": "themoviedb", "media_id": "100",
            "type": "电视剧", "season": "S01",
            "target_episodes": [1],
