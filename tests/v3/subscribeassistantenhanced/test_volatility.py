...
class TestVolatilityTracker:
    ...
    def test_recent_change_detail_rejects_reused_subscribe_identity(self):
        ...
        now = time.time()
        old = SimpleNamespace(
            id=41, media_source="themoviedb", media_id="100", season=1, episode_group=None
        )
        new = SimpleNamespace(
            id=41, media_source="themoviedb", media_id="200", season=1, episode_group=None
        )
    ...
    def test_legacy_recent_total_change_survives_subscribe_object_migration(self):
        ...
        assert entry["identity"]["media_source"] == "themoviedb"
        assert entry["identity"]["media_id"] == "100"
        assert entry["last_total_changed_at"] is not None
        assert entry["unstable_until"] is not None
    ...
    def test_reused_id_with_different_media_starts_new_history(self):
        ...
        new = SimpleNamespace(
            id=41, media_source="themoviedb", media_id="200", season=2, episode_group=None
        )
        ...
        entry = self.store["volatility"]["41"]
        assert entry["identity"]["media_source"] == "themoviedb"
        assert entry["identity"]["media_id"] == "200"
