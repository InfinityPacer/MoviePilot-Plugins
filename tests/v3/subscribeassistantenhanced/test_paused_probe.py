...
def _sub(sid=1, state="S", best_version=0, media_type=MediaType.TV):
    """构造暂停订阅替身。"""
    return SimpleNamespace(
        id=sid,
        name=f"测试{sid}",
        media_source="themoviedb",
        media_id=str(100 + sid),
        season=1,
        episode_group=None,
        type=media_type,
        state=state,
...
def test_music_subscription_is_rejected_before_pause_adoption():
    """音乐订阅不进入暂停登记、下载探测或补搜调度。"""
    music = _sub(media_type=MediaType.MUSIC)
    coordinator, data, _oper, chain, pause, monitor = _coordinator(
        [music],
        {music.id: None},
    )
    ...
    pause.get_pause_record.assert_not_called()
    pause.adopt_external.assert_not_called()
    monitor.has_active_downloads.assert_not_called()
    chain.search.assert_not_called()
    assert FakeTimer.instances == []
    assert data["subscribes"] == {}
