...
def _ctx(
    ...
    return SimpleNamespace(
        ...
        media_info=SimpleNamespace(
            tmdb_id=tmdbid, douban_id=doubanid, media_source=media_source,
            media_id=media_id, season=season, type="电视剧",
        ),
...
def test_explicit_id_conflict_becomes_conflict_even_when_tmdb_matches():
    evidence = classify_site_contexts(
        _sub(total_episode=12, media_source="douban", media_id="douban-a"),
        [_ctx(tmdbid=100, doubanid="douban-b", media_source="douban", media_id="douban-b", site_total=12)],
        now=_now(),
    )
...
def test_meta_explicit_id_conflict_becomes_conflict_even_when_media_tmdb_matches():
    evidence = classify_site_contexts(
        _sub(total_episode=12, media_source="douban", media_id="douban-a"),
        [_ctx(tmdbid=100, doubanid=None, explicit_doubanid="douban-b",
              media_source="douban", media_id="douban-b", site_total=12)],
        now=_now(),
    )
...
def test_snapshot_ignores_row_with_stale_identity():
    store = SiteEvidenceStore(_task_manager())
    subscribe = _sub(id=7, media_id="100", season=1, total_episode=12)
    stale_subscribe = _sub(id=7, media_id="200", season=1, total_episode=12)
...
def test_snapshot_ignores_row_with_stale_episode_group_identity():
    store = SiteEvidenceStore(_task_manager())
    subscribe = _sub(id=7, media_id="100", season=1, episode_group="group-a", total_episode=12)
    stale_subscribe = _sub(id=7, media_id="100", season=1, episode_group="group-b", total_episode=12)
    store.save_snapshot(stale_subscribe, SiteEvidence.no_evidence(stale_subscribe, now=_now()))

    assert store.read_snapshot(subscribe) is None


def test_applied_marker_clears_when_media_identity_changes():
    store = SiteEvidenceStore(_task_manager())
    subscribe = _sub(id=7, media_source="douban", media_id="douban-a", total_episode=12)
    store.mark_applied(subscribe, applied_total=12, applied_base_total=10, reason="site_total_ahead")

    subscribe.media_id = "douban-b"
...
def test_saving_snapshot_for_new_season_identity_drops_stale_applied_marker():
    store = SiteEvidenceStore(_task_manager())
    subscribe = _sub(id=7, media_id="100", season=2, total_episode=12)
    stale_subscribe = _sub(id=7, media_id="100", season=1, total_episode=12)
    store.mark_applied(stale_subscribe, applied_total=12, applied_base_total=10, reason="site_total_ahead")
...
def test_saving_snapshot_for_new_type_identity_drops_stale_applied_marker():
    store = SiteEvidenceStore(_task_manager())
    subscribe = _sub(id=7, media_id="100", season=1, type="电视剧", total_episode=12)
    stale_subscribe = _sub(id=7, media_id="100", season=1, type="电影", total_episode=12)
    store.mark_applied(stale_subscribe, applied_total=12, applied_base_total=10, reason="site_total_ahead")
...
def test_refresh_handler_ignores_expired_or_identity_mismatch_evidence():
    ...
    stale_subscribe = _sub(id=7, media_id="200", total_episode=10)
    mismatch_store.save_snapshot(stale_subscribe, _site_total_ahead(site_total=12, media_id="200"))
