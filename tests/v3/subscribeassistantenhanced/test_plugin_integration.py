...
def test_best_version_check_logs_actionable_recognition_failure(monkeypatch):
    ...
    sub = _sub(id=42, name="识别失败剧", best_version=1, best_version_full=0,
               media_source="themoviedb", media_id="100", season=1, type="电视剧")
    plugin._subscribe_oper = MagicMock()
    plugin._subscribe_oper.list.return_value = [sub]
    ...
    assert any(
        ...
        and "媒体身份：themoviedb:100" in message
        and "建议检查订阅名称、年份、媒体来源和媒体 ID、媒体类型和季号" in message
        for message in messages
    )


def test_meta_check_skips_music_subscriptions_before_lifecycle():
    """定时订阅遍历只把电影和电视剧交给生命周期协调器。"""
    movie = _sub(id=1, type=MediaType.MOVIE, season=None)
    tv = _sub(id=2, type=MediaType.TV)
    music = _sub(id=3, type=MediaType.MUSIC, season=None)
    plugin = SubscribeAssistantEnhanced()
    plugin.init_plugin({})
    plugin._subscribe_oper = MagicMock()
    plugin._subscribe_oper.list.return_value = [movie, tv, music]
    lifecycle = plugin._modules["lifecycle"]
    lifecycle.handle_meta_check_subscription = MagicMock()

    plugin.run_meta_check()

    assert lifecycle.handle_meta_check_subscription.call_args_list == [
        call(movie),
        call(tv),
    ]
...
class TestPeriodicJobs:
    ...
    def test_completion_rebuilder_is_wired_into_verifier(self):
        """入口装配的 verifier 使用同一重建组件完成创建与结果校验。"""
        plugin = SubscribeAssistantEnhanced()
        plugin.init_plugin({})
        rebuilder = plugin._modules["completion_rebuilder"]
        verifier = plugin._modules["verifier"]

        assert verifier._rebuild_subscribe == rebuilder.rebuild
        assert verifier._validate_rebuild_subscribe == rebuilder.validate
        assert rebuilder._subscribe_chain is plugin._subscribe_chain
        assert rebuilder._subscribe_oper is plugin._subscribe_oper
    ...
    def test_completion_verify_keeps_snapshot_for_lagging_episode_best_version_subscription(self):
        ...
        plugin._modules["verifier"]._subscribe_oper.list.return_value = [
            _sub(id=7, media_id="100", season=1, best_version=1, best_version_full=0)
        ]
    ...
    def test_completion_verify_keeps_snapshot_for_lagging_normal_subscription(self):
        ...
        plugin._modules["verifier"]._subscribe_oper = MagicMock()
        plugin._modules["verifier"]._subscribe_oper.list.return_value = [
            _sub(id=7, media_id="100", season=1, best_version=0, best_version_full=0)
        ]
        plugin._modules["verifier"]._rebuild_subscribe = MagicMock(return_value=True)
    ...
    def test_completion_verify_replaces_existing_full_best_version_subscription(self):
        ...
        plugin._modules["verifier"]._subscribe_oper = MagicMock()
        plugin._modules["verifier"]._subscribe_oper.list.return_value = [
            _sub(id=7, media_id="100", season=1, best_version=1, best_version_full=1)
        ]
        plugin._modules["verifier"]._rebuild_subscribe = MagicMock(return_value=True)
...
class TestNoDownloadCheck:
    ...
    def test_recognize_mediainfo_skips_unknown_media_type(self):
        ...
        subscribe = _sub(type=MediaType.UNKNOWN, name="测试", year="2025", season=1)
        plugin = SubscribeAssistantEnhanced()
        plugin.init_plugin({})
        plugin.chain = MagicMock()
    ...
    def test_last_download_date_queries_tv_history_and_returns_latest_date(self):
        ...
        subscribe = _sub(type="电视剧", name="测试", year="2025", season=1)
        plugin = SubscribeAssistantEnhanced()
        plugin.init_plugin({})
        plugin._downloadhistory_oper = MagicMock()
    ...
    def test_last_download_date_returns_none_when_history_query_fails(self):
        """下载历史查询异常时安全返回 None。"""
        subscribe = _sub(type="电影", name="测试", year="2025")
        plugin = SubscribeAssistantEnhanced()
        plugin.init_plugin({})
        plugin._downloadhistory_oper = MagicMock()
    ...
    def test_related_episode_download_histories_filters_full_pack_and_source(self):
        ...
        episode_download = SimpleNamespace(
            ...
            note={"source": 'Subscribe|{"id":18,"media_source":"themoviedb","media_id":"100","year":"2025","season":1}'},
            episode_group=None,
        ...
        other_subscribe = SimpleNamespace(
            ...
            note={"source": 'Subscribe|{"id":99,"media_source":"themoviedb","media_id":"100","year":"2025","season":1}'},
            episode_group=None,
