...
class TestWebhookNotify:
    """Webhook 请求校验、公共 API_TOKEN 认证和通知转发契约。"""
    ...
    def test_api_registers_get_and_post_with_public_token_dependency(self):
    ...
    def test_missing_token_is_rejected_and_valid_token_forwards_message(self, monkeypatch):
        plugin = WebhookNotify()
        ...
        authorized = client.post("/webhook?token=unit-test-token", json=payload)
    ...
    def test_post_accepts_title_or_body(self, monkeypatch, payload, expected_title, expected_text):
        ...
        response = client.post("/webhook?token=unit-test-token", json=payload)
    ...
    def test_get_and_post_reject_missing_content(self, monkeypatch):
        ...
        post_response = client.post("/webhook?token=unit-test-token", json={})
        get_response = client.get("/webhook?token=unit-test-token")
