...
def _settings(mode="balanced", keyword_config="", custom_config=""):
    return RecognitionSettings(mode=mode, keyword_config=keyword_config, custom_config=custom_config)
...
def test_audit_mode_records_would_action_without_changing_selection():
    guard = RecognitionGuard(
        _settings(mode="audit", custom_config="actions:\n  target_range_oversized: block\n")
    )
    subscribe_oper = MagicMock()
    subscribe_oper.get.return_value = _sub(start_episode=8, total_episode=19)
    context = _real_context("将夜 全60集", range(1, 61))
    data = ResourceSelectionEventData(origin='Subscribe|{"id": 1}', contexts=[context])
    ...
    assert guard.last_batch.recognition_output_count == 1
    assert guard.last_batch.final_count == 1
    assert guard.last_batch.decisions[0].would_action == "block"
    assert guard.last_batch.decisions[0].final_action == "observe"


def test_delete_fingerprint_updates_final_count_without_changing_recognition_output_count():
    guard = RecognitionGuard(_settings(mode="balanced"))
    subscribe_oper = MagicMock()
    subscribe_oper.get.return_value = _sub(start_episode=8, total_episode=19)
    deletes_store = MagicMock()
    deletes_store.match.side_effect = lambda enclosure=None, page_url=None: enclosure == "delete-me"
    contexts = [
        _real_context("将夜 第8集", [8], enclosure="delete-me", year="2026"),
        _real_context("将夜 第9集", [9], enclosure="keep-me", year="2026"),
    ]
    data = ResourceSelectionEventData(origin='Subscribe|{"id": 1}', contexts=contexts)
    ...
    assert data.updated is True
    assert data.updated_contexts == [contexts[1]]
    assert guard.last_batch.recognition_input_count == 2
    assert guard.last_batch.recognition_output_count == 2
    assert guard.last_batch.final_count == 1
    assert "stage=delete_fingerprint input=2 output=1" in guard.last_audit_summary
...
def test_recognition_notify_off_suppresses_notification_payload():
    guard = RecognitionGuard(_settings(
        mode="balanced",
        custom_config="actions:\n  target_range_oversized: block\n",
    ))
    guard.settings.notify_mode = "off"
    subscribe = _sub(id=1, name="将夜")
    guard.filter_candidate_dicts(
        {"name": "将夜", "media_type": "电视剧", "target_episodes": [8, 9], "range_confidence": "high"},
        [{"title": "将夜 全60集", "episodes": list(range(1, 61))}],
        [object()],
        selection_original_count=1,
    )

    assert guard.notification_payload(subscribe) is None
    assert "target_range_oversized" in guard.last_audit_summary


def test_recognition_notify_summary_ignores_observe_but_all_includes_it():
    subscribe = _sub(id=1, name="将夜")
    observe_guard = RecognitionGuard(RecognitionSettings(mode="balanced", notify_mode="summary"))
    observe_guard.filter_candidate_dicts(
        {"name": "将夜", "media_type": "电视剧", "target_episodes": [8], "range_confidence": "high"},
        [{"title": "将夜 S01E08", "episodes": [8], "year": None}],
        [object()],
        selection_original_count=1,
    )
    all_guard = RecognitionGuard(RecognitionSettings(mode="balanced", notify_mode="all"))
    all_guard.filter_candidate_dicts(
        {"name": "将夜", "media_type": "电视剧", "target_episodes": [8], "range_confidence": "high"},
        [{"title": "将夜 S01E08", "episodes": [8], "year": None}],
        [object()],
        selection_original_count=1,
    ...
    assert observe_guard.notification_payload(subscribe) is None
    assert all_guard.notification_payload(subscribe) is not None


def test_recognition_guard_audit_is_generated_when_notification_dispatch_is_disabled():
    guard = RecognitionGuard(RecognitionSettings(
        mode="balanced",
        notify_mode="summary",
        keyword_config="hard_block:\n  - 强制错误\n",
    ))
    subscribe_oper = MagicMock()
    subscribe_oper.get.return_value = _sub()
...
def test_recognition_guard_notification_rate_limit_does_not_suppress_audit():
    guard = RecognitionGuard(RecognitionSettings(mode="balanced", notify_mode="summary"))
    subscribe = _sub(id=1)
