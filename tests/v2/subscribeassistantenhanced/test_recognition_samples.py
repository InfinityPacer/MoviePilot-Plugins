...
def _guard(mode="balanced", custom_config=""):
    return RecognitionGuard(RecognitionSettings(mode=mode, custom_config=custom_config))
...
def test_ever_night_full_60_can_be_configured_as_unrecoverable_block():
    guard = _guard(custom_config="actions:\n  target_range_oversized: block\n")
    target = {
        "name": "将夜",
        "media_type": "电视剧",
        "shape": "animation",
    ...
    candidate = {
        "title": "Ever Night S01 全60集",
        "description": "将夜 全60集",
        "episodes": list(range(1, 61)),
        "year": None,
    }

    batch = guard.filter_candidate_dicts(target, [candidate], [object()], selection_original_count=1)
...
def test_ever_night_full_60_non_recoverable_soft_block_is_not_restored():
    guard = _guard(custom_config="empty_pool:\n  non_recoverable_codes:\n    - target_range_oversized\n")
    target = {
        "name": "将夜",
        "media_type": "电视剧",
        "shape": "animation",
