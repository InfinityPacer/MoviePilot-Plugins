    ...
    def handle_refresh(self, data: SubscribeEpisodesRefreshEventData) -> None:
        ...
        if subscribe is None:
            detail(
                ...
                f"media={data.media_source}:{data.media_id}, season={data.season}），跳过站点证据消费"
            )
            return
...
def _identity_matches(subscribe, context) -> bool:
    ...
    if not media_source or not media_id:
        return False
    context_pairs = {
        (str(source or ""), str(identifier or ""))
        for source, identifier in (
            (getattr(meta_info, "media_source", None), getattr(meta_info, "media_id", None)),
            (getattr(media_info, "media_source", None), getattr(media_info, "media_id", None)),
        )
        if source and identifier
    }
    if context_pairs:
        return (media_source, media_id) in context_pairs
    ...
    if media_source == MediaSource.TMDB.value:
        return media_id in context_tmdbids
    if media_source == MediaSource.Douban.value:
        return media_id in context_doubanids
    return False
...
def _identity(subscribe) -> dict:
    return {
        ...
        "episode_group": _normalize_text(subscribe.episode_group),
        "type": subscribe.type,
    }
...
def _row_identity_matches(row: dict, subscribe) -> bool:
    ...
    if not isinstance(identity, dict):
        return False
    expected_episode_group = _normalize_text(subscribe.episode_group)
    return (
        str(identity.get("media_source") or "") == str(subscribe.media_source or "")
        and str(identity.get("media_id") or "") == str(subscribe.media_id or "")
        and _safe_int(identity.get("season")) == _safe_int(subscribe.season)
        and _normalize_text(identity.get("episode_group")) == expected_episode_group
        and str(identity.get("type") or "") == str(subscribe.type or "")
    )
...
def _event_identity_matches(data: SubscribeEpisodesRefreshEventData, subscribe) -> bool:
    if data.season is not None and _safe_int(data.season) != _safe_int(getattr(subscribe, "season", None)):
        return False
    if data.media_source is not None and str(data.media_source) != str(subscribe.media_source):
        return False
    if data.media_id is not None and _normalize_id(data.media_id) != _normalize_id(subscribe.media_id):
        return False
    return True
...
def _scope_from_mediainfo(subscribe, mediainfo) -> Optional[SeasonScope]:
    ...
    if not episodes:
        return None
    return SeasonScope(
        tmdbid=subscribe_tmdb_id(subscribe) or 0,
        season=season,
