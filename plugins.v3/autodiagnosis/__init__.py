...
class AutoDiagnosis(_PluginBase):
    ...
    def init_plugin(self, config: Optional[dict] = None):
        ...
        self._execute_when_system_error = bool(
            config.get("execute_when_system_error", False)
        )
        self._notify = config.get("notify", "on_error")
        self._notify_type = config.get("notify_type", "Plugin")
        self._health_check_modules = config.get("health_check_modules") or []
        self._health_check_sites = config.get("health_check_sites") or []
        self._history_link_check = config.get("history_link_check")
        self._history_link_mode = config.get("history_link_mode", "link")
        self._dir_link_check = config.get("dir_link_check")
    ...
    def stop_service(self):
        ...
        try:
            ...
            logger.warning("停止自动诊断任务失败：%s", err)
        finally:
            self._event.clear()
    ...
    def __resolve_results(self, results_dict):
        ...
        if self._notify == "always" or (self._notify == "on_error" and any_errors):
            if message:
                notify_type = (
                    NotificationType.__members__.get(self._notify_type)
                    if isinstance(self._notify_type, str)
                    else None
                )
                if notify_type is None:
                    notify_type = NotificationType.Plugin
                self.post_message(mtype=notify_type, title="【自动诊断】", text=message)
    ...
    def __check_history_link(self) -> List[Dict[str, Any]]:
        ...
        if self._history_link_check == "since_last":
            ...
            link_items = self.__list_by_date_for_link(date=since_last_time)
        elif self._history_link_check == "all":
            link_items = self.__list_by_date_for_link(date=None)
        else:
            link_items = self.__list_by_count_for_link(count=self._history_link_check)
    ...
    def __query_transfer_history(
        self,
        *,
        date: Optional[str] = None,
        count: Optional[int] = None,
    ) -> list[TransferHistorySnapshot]:
        """通过公开查询 SDK 分页读取成功整理历史，并保留历史模式筛选。"""
        if count is not None:
            try:
                count = int(count)
            except (TypeError, ValueError):
                logger.warning("历史记录检查数量无效：%s", count)
                return []
            if count <= 0:
                return []

        filters = TransferHistoryFilter(status=True)
        link_only = not self._history_link_mode or self._history_link_mode == "link"
        ...
        while True:
            ...
            for history in page.items:
                if date is not None and (
                    not history.date or history.date <= date
                ):
                    continue
                if link_only and history.mode != "link":
                    continue
                histories.append(history)
                if count is not None and len(histories) >= count:
                    return histories[:count]
            if not page.has_next:
                return histories
            page_number += 1

    def __list_by_count_for_link(
        self, count: int
    ) -> list[TransferHistorySnapshot]:
        """查询最近一定数量的成功整理历史。"""
        return self.__query_transfer_history(count=count)

    def __list_by_date_for_link(
        self, date: Optional[str]
    ) -> list[TransferHistorySnapshot]:
        """查询指定时间之后的成功整理历史。"""
        return self.__query_transfer_history(date=date)
