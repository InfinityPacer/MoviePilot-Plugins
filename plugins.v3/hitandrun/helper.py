...
class TorrentHelper:
    ...
    @staticmethod
    def _get_transmission_hash(torrent: Any) -> str:
        """读取 transmission-rpc 当前字段，并兼容旧版本对象。"""
    ...
    def get_torrent_info(self, torrent: Any) -> dict:
        ...
        # TR
            ...
            # ID
            torrent_id = self._get_transmission_hash(torrent)
            # 标题
    ...
    def get_torrent_hashes(self, torrents: Union[Any, List[Any]]) -> Union[str, List[str]]:
        ...
        try:
            if isinstance(torrents, list):
                ...
                return [
                    torrent.get("hash") if self.dl_type == "qbittorrent" else self._get_transmission_hash(torrent)
                    for torrent in torrents
                ]
            else:
                # 处理单个种子的情况，返回该种子的 hash
                return torrents.get("hash") if self.dl_type == "qbittorrent" else self._get_transmission_hash(torrents)
        except Exception as e:
            print(str(e))
