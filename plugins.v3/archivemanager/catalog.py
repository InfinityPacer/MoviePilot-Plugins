...
def _record_from_manifest(path: Path, folder: Path) -> dict:
    ...
    record = {
        ...
        # SHA-1 自 0.1.6 起写入，旧清单缺失时为空串；只用于与仅提供 SHA-1 的网盘对账。
        "archive_sha1": value.get("archive_sha1", ""),
...
def _record_markdown(batch: dict, record: dict) -> str:
    ...
    lines = [
        ...
        "",
        f"归档 SHA-1：`{record.get('archive_sha1') or '未记录'}`（仅用于与网盘元数据对账）",
        "",
    ...
    with _CATALOG_LOCK:
        ...
        record = {
            **manifest,
            ...
            "archive_sha1": batch.get("archive_sha1", ""),
