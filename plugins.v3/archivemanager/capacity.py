...
def allowance(
    task: TaskConfig, local: list[dict], daily_limit_bytes: int = 0, daily_used_bytes: int = 0
) -> dict:
    """源数据按最坏的仅打包大小预留；压缩节省不能提前当作可用空间。"""
    free = available_space(Path(task.output_dir))
    size = sum(
        Path(batch["archive_path"]).stat().st_size
        for batch in local
        if Path(batch["archive_path"]).is_file()
    )
    count = len(local)
    overhead = 64 * 1024**2
    ...
    reason = ""
    if daily_limit_bytes and daily_used_bytes >= daily_limit_bytes:
        reason = "今日归档额度已用完，等待次日恢复"
        budget = 0
    elif task.max_pending_archives and count >= task.max_pending_archives:
    ...
    return {
        ...
        "budget": budget,
        "reason": reason,
        "daily_archive_bytes": daily_used_bytes,
        "daily_archive_limit_bytes": daily_limit_bytes,
    }
