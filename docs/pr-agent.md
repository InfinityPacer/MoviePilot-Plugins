...
当前验证阶段仅自动处理仓库所有者本人从同仓发起并触发的 PR，用于确认新的 runner 集成可以在本 PR 中完整发布中文审查结果。合入前会在同一个 PR 中恢复为仅自动处理 fork PR。
...
workflow 不会 checkout 或执行 PR 分支代码。当前验证路径同时限制 PR 来源、作者和触发者均为仓库所有者，并使用固定到已验证 digest 的发布版本。权限保持最小化：只授予读取仓库内容所需的 `contents: read`，以及更新 PR Body、发布 Review 和回复 PR 评论所需的写权限；不会向仓库推送代码或创建提交。
